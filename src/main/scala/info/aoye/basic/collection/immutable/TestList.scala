...
object TestList extends scala.App {
  ...
  // 在列表中添加新对象
  // 该操作将一个新对象添加到本列表的最前面,注意：需要定义一个新的List接收返回值
  ...
  list5.head //返回列表的第一个元素
  list5.tail //返回除列表第一个元素组成的新列表, 如果列表中仅有一个元素,这里返回Nil
  ...
  /**
    ...
    * 不推荐使用.
    */
  val linkedList = scala.collection.mutable.LinkedList
