...
//注意:上述三个语句的顺序藏着一点玄机,我们知道,通常,如果import进来两个包都有某个类型的定义的话,比如说,同一段程序,即引用了'scala.collection.mutable.Set'又引用了'import scala.collection.immutable.Set'
//     则编译器会提示无法确定用哪一个Set.这里的隐式引用则不同,如果有相同的类型,后面的包的类型会将前一个隐藏掉.
//     比如java.lang和scala两个包里都有StringBuilder这种情况下会使用scala包里定义的那个.java.lang里的定义就被隐藏掉了,除非显示的使用java.lang.StringBuilder
